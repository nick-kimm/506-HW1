...
isPalindromic <- function(x){
  ...
  forward = character(length(x))
  backward = character(length(x))
  for (i in 1:length(digits)){
    forward[i] = digits[i]
    backward[length(digits)-i+1] = digits[i]
    rev = as.numeric(backward)
  }
  if (forward[i]==backward[i]){
    return(list(isPalindromic=isP,reversed=rev))
  }
  else{
    isP = FALSE
    return(list(isPalindromic=isP,reversed=rev))
...
nextPalindromic <- function(x){
  ...
  new_num = character(length(x)) #this is the next Palindrome number we will fill in the values for 
  
  while (new_num < x){
    if (length(digits)%%2==0){ #if length is even. Logic goes look at first half and up last term by 1 and flip for other half
...
```{r}
n = 1000
test <- unlist(strsplit(as.character(n), ""))
